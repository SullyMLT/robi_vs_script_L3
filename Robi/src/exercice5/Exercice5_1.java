...
public class Exercice5_1 {
	...
	public Exercice5_1() {
		...
		ajoutComponent("space", new CRect(0, 0, 0, 0));
		//this.mainLoop();
	}
	...
	public class CRect extends Component{
		int w;
		int h;
	...
	public void oneShot(String scripts) {
			...
			while (itor.hasNext()) {
				...
				for (int i = 0; i < s.children().size(); i++) {
					//System.out.print(s.get(i).contents().toString() + " ");
					String name = s.get(0).contents().toString();
					if (componentExistsByName(name)) {
						System.out.println("le component : " + name + " existe");
					} else {
						System.out.println("Ajout du component : " + name);
						ajoutComponent(name, new Object());
					}
					Reference currentRef = new Interpreter().compute(environment, itor.next());
					System.out.print("currentRef" + currentRef.toString());
				}
				System.out.println("");
			}
	}
	public void ajoutComponent(String name, Object obj) {
		components.put(name, obj);
	}
	
	public boolean componentExistsByName(String name) {
	...
	public class SetDim implements Command {
	    public Reference run(Reference ref, SNode methode) {
	       ...
	       /* // V�rifie d'abord si le receiver est une instance de GSpace
	        ...
	        // Si le receiver n'est ni un GSpace ni un GBounded, imprime un message d'erreur.
	        else {
	            System.err.println("Erreur: L'objet " + receiver.getClass().getName() + " ne peut pas �tre redimensionn�.");
	        } */
